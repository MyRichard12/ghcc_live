  ...
  return (
    <section
      className={`z-0 ${setup?.align == "left" && `flex flex-wrap`} ${
        ...
        `flex ${
          setup.hasImage
            ? `lg:flex-row-reverse md:flex-row-reverse flex-wrap`
            : `flex-row-reverse`
        }`
    ...
    >
      <div
        className={`${
          (!setup.hasImage && `hidden lg:p-20 bg-neutral-800`) || `lg:px-10`
        } lg:flex z-[2] items-center justify-center p-2 w-full lg:w-1/3`}
      >
        {/* grid image flows here */}
        {setup?.hasImage === true &&
        setup?.hasImage === true &&
        setup?.staticImage === true ? (
      ...
      <div
        className={`z-[1] ${
          !setup.hasImage && `bg-slate-100 lg:bg-slate-100`
        } w-full lg:w-2/3 flex flex-shrink-0 flex-col justify-center gap-y-1 p-2 lg:p-20 text-sm font-normal lg:text-xl leading-6 text-zinc-700 shadow-none`}
      >
        {typeof setup.content === "object" &&
          setup.content.length > 0 &&
          setup?.content.map((value, index) => {
            return (
              <div
                key={index}
              ...
              >
                <div className="w-full">
                  ...
                  <div className="flex flex-col items-center lg:items-start lg:justify-start pt-4 mb-4">
                    ...
                    {value?.hasMore === true && (
                      <AddMore text={`Learn more`} href="/about-us" />
                    )}
                  </div>
                </div>
        ...
        <div class="row gx-5 mt-12 items-center">
          <div class="mt-6 lg:col-5 lg:order-1 lg:mt-0">
            <div class="text-container">
              <h2 class="lg:text-4xl">Add a New Employee</h2>
              <div class="mt-6 text-dark lg:-ml-4">
                <p class="mb-2 flex items-center rounded px-4">
                  This feature simplifies the onboarding process, allowing you
                  to integrate new employees into your practice seamlessly and
                  ensuring that everyone is equipped to provide top-notch care
                  to clients.{" "}
                </p>
              </div>
            </div>
        ...
        <div className="w-full grid grid-cols-1  md:grid-cols-3 gap-10 md:px-20 px-4">
          <div className="w-full flex flex-col">
            ...
            <div className="h-[350px] w-full mb-4 border-2">
              <img
                src={Purp}
                alt="Discovering Purpose"
                className="w-full h-full object-cover"
              />
            </div>
            <div className="text-black font-bold text-lg mb-2">
              Discovering Purpose: What is God’s purpose for me?
            </div>
            <div className="text-xs font-normal text-black mb-2">
              on <span className="font-medium pr-3">Jan 26, 2024</span> by{" "}
              <span className="pl-1 font-medium">GHCC Scribe</span>
            </div>
            <AddMore
              text={`Read More`}
              href="https://medium.com/@gloryhillscommunitychurch/discovering-purpose-what-is-gods-purpose-for-me-a6478747a751"
            />
          </div>
          ...
          <div className="w-full flex flex-col">
            ...
            <div className="text-xs font-normal text-black mb-2">
              on <span className="font-medium pr-3">April 26, 2024</span> by{" "}
              <span className="pl-1 font-medium">GHCC Scribe</span>
            </div>
            <AddMore
              text={`Read More`}
          ...
          <div className="w-full flex flex-col">
            ...
            <div className="text-black font-bold text-lg mb-2">
              The Cost of Discipleship for the Believer
            </div>
            <div className="text-xs font-normal text-black mb-2">
              on <span className="font-medium pr-3">Jun 26, 2024</span> by{" "}
              <span className="pl-1 font-medium">GHCC Scribe</span>
            </div>
            <AddMore
              text={`Read More`}
              href="https://medium.com/@gloryhillscommunitychurch/the-cost-of-discipleship-for-the-believer-a3bdd39e54d3"
            />
          </div>
        </div>
      )}
        ...
        setup?.content.map((value, index) => {
          return (
            <div className="w-full flex flex-col">
              ...
              <div className="text-sm mb-2 font-medium underline text-sky-600">
                { value.link != null && value.link != '#' && <a href={value.link}>Register here</a>}
              </div>
            </div>
          );
        })}
            ...
            return (
              <div key={index} className="w-full flex flex-col">
                ...
                <GatsbyImage
                  ...
                  alt={data.name}
                />
                <div className="text-sky-700 text-sm leading-tight tracking-tight md:text-lg font-bold">
                  {data.name}
                </div>
                <div className="text-xs md:text-md font-normal mb-1">
                  {data.title}
                </div>
          ...
          {setup.images.map((data, index) => {
            return (
              <div
                ...
                className="w-full h-[500px] flex flex-col rounded-xl relative"
              >
                {/* the image */}
                <div className="text-black font-bold text-md bg-opacity-80 bg-blue-900 p-3 flex flex-col absolute bottom-6">
                  <span className="text-white text-sm mb-2">{data.quote}</span>
        ...
        setup.images.map((data, index) => {
          return (
            <div
              ...
              style={{
                backgroundImage: `url(${
                  data?.image?.childImageSharp?.fluid?.src || data.imageData
                })`,
                backgroundSize: "fill",
                backgroundRepeat: "no-repeat",
              }}
              className="w-full h-[400px] flex flex-col rounded-xl relative"
            >
              {/* the image */}
              <div className="text-black font-bold text-md bg-blue-900 p-3 flex flex-col absolute bottom-0">
                ...
                <span className="text-white text-sm">{data.quote}</span>
              </div>
            </div>
          );
        })}
